...
{
	FB.init({appId: "328608114173164", cookie: true, xfbml: true, version: "v2.8"});
...
{
	ConsoleLog("[Facebook] StatusChangeCallback (Response: " + JSON.stringify(response) + ")");
	...
	else
	{
		...
		ConsoleLog("[Facebook] Not logged into Facebook");
		return null;
	}
}
...
{
	ConsoleLog("[Facebook] Fetching logged in person information...");
	FB.api("/me", function(response)
	{
		ConsoleLog("[Facebook] Successful login for: " + response.name);
	});
}
...
Facebook.prototype.FacebookAppLogin = function(AfterLogin)
{
	...
	facebook.CheckLoginState(function(facebookAccessToken)
	{
		...
		{
			ConsoleLog("[Facebook] Ask user to log in...");
